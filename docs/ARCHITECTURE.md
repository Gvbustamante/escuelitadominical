...
| T4 | **La pertenencia al mismo programa se garantiza con disparadores en la base**, no solo con RLS | Una clave foránea a `profiles` garantiza que el perfil existe, no que sea del mismo programa. Los disparadores `validar_tenant_*` cierran esa clase entera (líder↔diplomado, docente↔módulo, estudiante↔matrícula, estudiante y diplomado↔certificado, responsable↔tarea), de modo que ni una política mal escrita ni una función `security definer` futura puedan cruzar programas por descuido. Nace de la lección de T2: la RLS sola resultó ser una capa demasiado fácil de saltarse sin querer. |
